...
namespace structures
{
	...
	{
		// NOTE: ak je utriedený delím na polovicu
		// TODO: dorobiť podľa fotky
		int index = -1;
		if (list_->size() >= 1) {
			for (PriorityQueueItem<T> * item : *list_) {
				if (priority >= item->getPriority()) {
					index = this->list_->getIndexOf(item);
					break;
				}
			}
		}
		this->list_->insert(new PriorityQueueItem<T>(priority, data), index == -1 ? static_cast<int>(list_->size()) : index);
	}
	...
	inline int PriorityQueueSortedArrayList<T>::indexOfPeek() const
	{
		if (list_->size() <= 0)
		{
			throw std::logic_error("PriorityQueueSortedArrayList<T>::indexOfPeek(): Prierity Que is empty!");
		}
